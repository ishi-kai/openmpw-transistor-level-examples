...
module SAR_LOGIC #(
    ...
    localparam [2:0] ST_APPLY_DAC = 3'd1;  // 試行する DAC コードを出す
    ...
    // 最初に試す DAC コード。
    // 6bit の場合は 7bit 幅の 1000000 になり、MSB 側だけを 1 にする。
    localparam [BIT_ADC:0] MSB_TRIAL = (1'b1 << BIT_ADC);
    ...
    always @(posedge CLK or negedge XRST) begin
        if (XRST == LOW) begin
            ...
            case (state)
                ...
                ST_APPLY_DAC: begin
                    // CDAC の電荷を保持し、現在の試行コードを出す。
                    // MSB 判定のときだけ、最初の 1000000 を出す。
                    COMP_CLK <= LOW;
                    SC <= LOW;
                    EOC <= LOW;
